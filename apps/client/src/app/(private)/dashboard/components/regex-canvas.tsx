...
export function RegexCanvas({
	...
	const [draggingCanvasIndex, setDraggingCanvasIndex] = useState<number | null>(null)
	...
	const handleDrop = (e: React.DragEvent, insertAt: number, draggedData: string) => {
		e.preventDefault()
		onDragOverIndex(null)

		try {
			const data = JSON.parse(draggedData || e.dataTransfer.getData('text/plain'))
		...
		<div className="flex flex-col gap-2 h-full">
			...
			<div
				...
				style={{
					...
					background: dragOverIndex !== null ? 'rgba(6,182,212,0.03)' : 'rgba(255,255,255,0.05)',
				}}
			>
				{elements.length === 0 && (
				...
				<div className="flex flex-wrap gap-1.5 items-center">
						...
						{elements.map((el, idx) => (
							<div key={el.id} className="flex items-center">
								...
								<DropZone
									...
									onDrop={(e) => handleDrop(e, idx, e.dataTransfer.getData('text/plain'))}
								/>
								...
								>
									<CanvasElement
										...
										onDrop={(e) => handleDrop(e, idx + 1, JSON.stringify({ ...el, _canvasIndex: idx }))}
									/>
								</motion.div>
							</div>
						))}
					...
					{elements.length > 0 && (
						<DropZone
							...
							onDrop={(e) => handleDrop(e, elements.length, e.dataTransfer.getData('text/plain'))}
						/>
					)}
				</div>
			</div>
...
function CanvasElement({
	...
	const handleDragStart = (e: React.DragEvent) => {
		e.dataTransfer.setData('text/plain', JSON.stringify({ ...element, _canvasIndex: index }))
		onDragStart()
	}
	...
	return (
		<Tooltip delayDuration={200}>
				...
				>
					{element.configurable ? (
						<div className="flex items-center gap-1">
							<span className="text-xs opacity-60">{element.label.replace('…', '')}</span>
							<input
								value={element.input ?? ''}
							...
							/>
							{element.type === 'group' && <span className="text-xs opacity-60">)</span>}
						</div>
					) : (
			...
			<TooltipContent side="bottom" align="center" className="p-0 border-none bg-transparent shadow-none">
				<div
