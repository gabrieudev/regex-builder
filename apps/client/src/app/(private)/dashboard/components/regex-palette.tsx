...
const CATEGORY_ORDER = ['literal', 'charClass', 'quantifier', 'anchor', 'group', 'lookaround']

export function RegexPalette({ paletteByCategory, onDragStart, disabled, onItemClick }: Props) {
	return (
		<Accordion
			type="multiple"
						...
						<AccordionContent>
							<div className="p-2 flex flex-wrap gap-1.5">
								{paletteByCategory[cat].map((el) => (
									<PaletteItem key={el.id} element={el} onDragStart={onDragStart} disabled={disabled} onClick={onItemClick} />
								))}
							</div>
						</AccordionContent>
					</AccordionItem>
	...
	return (
		<Tooltip delayDuration={200}>
			...
			<TooltipContent side="bottom" align="center" className="p-0 border-none bg-transparent shadow-none">
				...
				>
					<div className="font-mono font-bold mb-0.5" style={{ color: element.color }}>
						{element.label}
					</div>
