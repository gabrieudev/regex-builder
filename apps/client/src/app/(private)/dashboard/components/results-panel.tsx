		...
		return (
			<div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 flex items-center justify-center">
				<div className="flex flex-col items-center gap-3 text-center">
					...
					/>
					<span className="text-xs font-mono text-gray-500 dark:text-gray-400">executando...</span>
				</div>
			</div>
		)
	}
		...
		return (
			...
			>
				<div className="flex items-center gap-2 mb-2">
					...
					<span className="text-xs font-mono font-bold text-red-600 dark:text-red-400">Erro</span>
				</div>
				<p className="text-xs font-mono text-red-600/80 dark:text-red-400/80">{error}</p>
			</motion.div>
		)
	}
			...
			>
				...
				>
					<div className="flex items-center gap-2">
						...
						/>
						<span className="text-xs font-mono font-bold" style={{ color: isSuccess ? '#10b981' : '#f43f5e' }}>
							{isSuccess ? 'Sucesso' : 'Falha'}
						</span>
						{result.error && <span className="text-xs font-mono text-red-500/80 dark:text-red-400/80">— {result.error}</span>}
					</div>
				...
				{result.matches?.length > 0 && (
					<ResultSection title="Correspondências" color="#06b6d4" count={result.matchCount}>
						...
						<div className="flex flex-col gap-1">
							{Object.entries(result.namedGroups).map(([name, vals]) => (
								<div key={name} className="flex items-center gap-2 flex-wrap">
									<span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400">{name}:</span>
									{vals.map((v, i) => (
										<span
											key={i}
				...
				{result.warnings?.length > 0 && (
					<ResultSection title="Avisos" color="#f59e0b">
						{result.warnings.map((w, i) => (
							<p key={i} className="text-xs font-mono text-amber-600/80 dark:text-amber-400/80">
	...
	return (
		<Card className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
			<CardHeader className="flex flex-row items-center gap-2 px-4 py-2 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
				...
				<CardTitle className="text-xs font-mono text-gray-500 dark:text-gray-400">{title}</CardTitle>
				{count !== undefined && (
			...
			<CardContent className="px-4 py-3 flex flex-col gap-2">{children}</CardContent>
		</Card>
	)
}
