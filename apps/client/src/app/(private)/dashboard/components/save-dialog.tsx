...
export function SaveDialog({ open, pattern, onSave, onClose, loading, initialName }: Props) {
			...
			<DialogContent className="sm:max-w-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
				...
				<div className="flex flex-col gap-4 py-4">
					<div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-3 py-2">
						<p className="text-[10px] font-mono text-gray-500 dark:text-gray-400 mb-1">padrão</p>
						<p className="font-mono text-xs text-cyan-600 dark:text-cyan-400 break-all">{pattern || '—'}</p>
					</div>
					<div>
						<label className="block text-xs font-mono text-gray-500 dark:text-gray-400 mb-2">nome</label>
						<Input
							value={name}
					...
					<Button
						...
						style={{
							background:
								name.trim() && pattern ? 'linear-gradient(135deg, rgba(6,182,212,0.1), rgba(37,99,235,0.1))' : undefined,
							borderColor: name.trim() && pattern ? 'rgba(6,182,212,0.3)' : undefined,
							color: name.trim() && pattern ? '#06b6d4' : undefined,
						}}
					>
						{loading ? <Loader2 className="animate-spin" /> : initialName ? 'Salvar alterações' : 'Salvar'}
					</Button>
				</div>
			</DialogContent>
		</Dialog>
