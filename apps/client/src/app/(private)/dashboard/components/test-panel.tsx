...
export function TestPanel({ testString, onChange, result, pattern, isLoading, onExecute }: Props) {
	const segments = useMemo(() => {
		if (!result?.success || !result.matchRanges?.length || !testString) return null
	...
	return (
		<div className="flex flex-col gap-3">
			...
			<Button
				...
				style={{
					background:
						!pattern || isLoading ? undefined : 'linear-gradient(135deg, rgba(6,182,212,0.1), rgba(37,99,235,0.1))',
					borderColor: !pattern || isLoading ? 'rgb(209 213 219 / 0.5)' : 'rgb(6 182 212 / 0.3)',
				...
				>
					<div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
						<div className="flex items-center gap-2">
							...
							<span className="text-xs font-mono text-gray-500 dark:text-gray-400">correspondências destacadas</span>
						</div>
