		...
		return (
			<div className="min-h-screen flex items-center justify-center">
				<div className="flex flex-col items-center gap-3">
					...
					<span className="text-sm font-mono text-gray-500">Carregando regex...</span>
				</div>
			</div>
		)
	}
		...
		<div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-200 flex flex-col">
			...
			>
				<div className="flex items-center gap-3">
					...
					<LanguageSelector language={dash.language} onChange={dash.setLanguage} />
					<Button
						size="sm"
				...
				<div className="flex items-center gap-2 md:hidden">
					...
					<Sheet open={dash.isReferenceOpen} onOpenChange={dash.setIsReferenceOpen}>
						<SheetTrigger asChild>
							<Button variant="outline" size="icon" className="h-8 w-8">
						...
						<SheetContent side="right" className="w-64 p-0">
							<SheetHeader className="p-4 border-b">
								<SheetTitle className="text-sm font-mono">Referência</SheetTitle>
							</SheetHeader>
							<div className="p-2 overflow-y-auto h-full">
			...
			{/* Layout principal */}
			<div className="relative z-10 flex flex-1 gap-0 overflow-hidden" style={{ height: 'calc(100vh - 65px)' }}>
				{/* Esquerda: Paleta */}
				<motion.aside
				...
				>
					<div className="px-3 py-3 border-b border-gray-200 dark:border-gray-800">
						<p className="text-[10px] font-mono text-gray-500 dark:text-gray-400 uppercase tracking-widest">Elementos</p>
					</div>
				...
				>
					...
					<section className="flex flex-col gap-2">
						<div className="flex items-center gap-2">
							<span className="text-[10px] font-mono text-gray-500 dark:text-gray-400 uppercase tracking-widest">
								{dash.mode === 'visual' ? 'Canvas de Construção' : 'Entrada Manual'}
							</span>
							<div className="flex-1 h-px bg-gray-200 dark:bg-gray-800" />
							{dash.mode === 'visual' && (
								<span className="text-[10px] font-mono text-gray-400 dark:text-gray-600">
									{dash.canvasElements.length} {dash.canvasElements.length === 1 ? 'elemento' : 'elementos'}
								</span>
							)}
						</div>
						<div style={{ minHeight: '120px' }}>
							{dash.mode === 'visual' ? (
								...
								<ManualRegexInput value={dash.manualPattern} onChange={dash.setManualPattern} />
							)}
						</div>
					</section>
					...
					<section className="flex flex-col gap-2">
						<div className="flex items-center gap-2">
							<span className="text-[10px] font-mono text-gray-500 dark:text-gray-400 uppercase tracking-widest">Padrão</span>
							<div className="flex-1 h-px bg-gray-200 dark:bg-gray-800" />
						</div>
						<RegexPreview pattern={dash.pattern} language={dash.language} codeSnippet={dash.codeSnippet} />
					</section>

					{/* Painel de teste */}
					<section className="flex flex-col gap-2">
						<div className="flex items-center gap-2">
							<span className="text-[10px] font-mono text-gray-500 dark:text-gray-400 uppercase tracking-widest">Teste</span>
							<div className="flex-1 h-px bg-gray-200 dark:bg-gray-800" />
						</div>
						<TestPanel
					...
					{/* Resultados */}
					{(dash.executeRegexMutation.data || dash.executeRegexMutation.error || dash.executeRegexMutation.isPending) && (
						<section className="flex flex-col gap-2">
							<div className="flex items-center gap-2">
								<span className="text-[10px] font-mono text-gray-500 dark:text-gray-400 uppercase tracking-widest">
									Resultados
			...
			<SaveDialog
				...
				loading={dash.createRegexMutation.isPending || dash.updateRegexMutation.isPending}
...
function QuickReference({ language }: { language: string }) {
	...
	return (
		<div className="p-3 flex flex-col gap-4">
			{tips.map((section) => (
				<Card key={section.title} className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
					<CardContent className="p-3">
						<p className="text-[9px] font-mono uppercase tracking-widest mb-2" style={{ color: section.color }}>
							{section.title}
						</p>
						<div className="flex flex-col gap-1">
							{section.items.map((item) => (
								<div key={item.label} className="flex items-center justify-between gap-2 py-1">
									<span className="text-[10px] font-mono font-bold" style={{ color: section.color }}>
										{item.label}
									</span>
									<span className="text-[10px] text-gray-500 dark:text-gray-400 text-right leading-tight">{item.pattern}</span>
								</div>
							))}
						</div>
					</CardContent>
			...
			<Card className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
				<CardContent className="p-3">
					<p className="text-[9px] font-mono uppercase tracking-widest mb-2 text-amber-600 dark:text-amber-400">Dicas</p>
					<div className="flex flex-col gap-2">
						{[
							...
							<p key={i} className="text-[10px] text-gray-500 dark:text-gray-400 leading-relaxed">
								{tip}
							</p>
						))}
					</div>
