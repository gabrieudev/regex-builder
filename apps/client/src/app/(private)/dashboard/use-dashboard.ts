...
const ELEMENT_PALETTE: RegexElement[] = [
  ...
  },
  {
    ...
    color: "#06b6d4",
    description:
      "Classe de caracteres — corresponde a qualquer caractere listado",
    configurable: true,
    placeholder: "a-z0-9",
    input: "",
  },
...
export function useDashboard() {
  ...
  const codeSnippet = useMemo(
    () => buildCodeSnippet(pattern, language),
    [pattern, language],
  );

  const addElement = useCallback(
    (paletteEl: RegexElement, insertAt?: number) => {
  ...
  const updateElementInput = useCallback((id: string, input: string) => {
    setCanvasElements((prev) =>
      prev.map((el) => (el.id === id ? { ...el, input } : el)),
    );
  }, []);
