...
function applyFilters(items: Regex[], filters: RegexFilters): Regex[] {
  return items.filter((item) => {
    ...
    if (
      filters.patternLike &&
      !item.pattern.toLowerCase().includes(filters.patternLike.toLowerCase())
    )
      return false;
    if (filters.patternExact && item.pattern !== filters.patternExact)
      return false;
...
export function useRegexManager() {
  ...
  async function fetchRegexes(params: typeof apiFilters) {
    ...
    if (params.dateFrom)
      query.append(
        "createdAtFrom",
        format(params.dateFrom, "yyyy-MM-dd'T'HH:mm:ss"),
      );
    if (params.dateTo)
      query.append(
        "createdAtTo",
        format(params.dateTo, "yyyy-MM-dd'T'HH:mm:ss"),
      );
  ...
  const updateFilter = useCallback(
    <K extends keyof RegexFilters>(key: K, value: RegexFilters[K]) => {
