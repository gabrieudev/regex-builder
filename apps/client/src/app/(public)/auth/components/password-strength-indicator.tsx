	...
	return (
		...
		>
			<div className="flex items-center justify-between text-sm">
				...
				<Badge variant="outline" className={score > 2 ? 'border-primary text-primary' : ''}>
					{label}
				</Badge>
			</div>

			<Progress value={percentage} className={`h-2 ${indicatorClassMap[mapKey]}`} />

			<div className="grid grid-cols-2 gap-1 text-xs">
				{requirements.map((req, idx) => (
					<div key={idx} className="flex items-center gap-1">
						{req.met ? <Check className="h-3 w-3 text-primary" /> : <X className="h-3 w-3 text-muted-foreground/50" />}
